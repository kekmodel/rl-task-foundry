...
def _too_easy_retry_guidance() -> str:
    return (
        ...
        "ADD exactly one new structural dimension to the current "
        "label without removing any existing structure. Replacing "
        "a field on the same path is not an escalation and will "
        "be rejected. Pick one axis the label does not yet have: "
        "(a) Width — pull more user-facing fields from another "
        "table along the path, "
        "(b) Filter — add a categorical exclusion or threshold, "
        "(c) Cardinality — return a list of N records (N ≥ 2) "
        "where N is declared in the question, "
