...
def build_synthesis_agent_instructions(
    ...
    return "\n\n".join([
        ...
        "2. Observe via `sample`/`profile`/`query`; `neighborhood` for "
        ...
        "row set, order, tie-breaks without technical or awkward controls. "
        "If not, choose another label.\n"
        "5. Derive `user_request` and `topic` from that label; request "
        "exactly the label fields and row controls.\n"
        "6. Final `query(spec)` answers hidden context before "
        "`submit_draft`, supplies copied label JSON, checks diagnostics; "
        ...
        "same role. If one phrase can map to several surfaces, name chosen "
        "source in request/contract or choose another label. If no primary key, "
        "use a primary-key-backed path/aggregate; no hidden path guessing. "
        "Shared everyday nouns need lifecycle/source role in user_request; do "
        "not depend on output_schema field names.",
        ...
        "global report.\n"
        "- Hidden-handle-only entity/filter: do not expose it; choose another path.\n"
        "- Copy visible context values only from latest scoped query evidence.\n"
        "- Keep request compact. Field keys stay in JSON; avoid aliases/choices "
        "in parentheses. If controls need long tie-breaks/mechanical field "
        "lists, choose another label.\n"
        "- Bind modifiers/filters to exact object/scope/source. Non-null "
        "filters need row-set wording, not output names. If parent/child rows "
        "can match, name the surface; avoid child-row-filter wording unless "
        "query uses it.",
        ...
        "- Source-sensitive fields/filters (status/type/category/frequency/"
        "stage/route/sequence/rank) need same source role used by the query "
        "path; request wording must name it; no normalized choices. "
        "Distinguish source sequence from display rank. Broad words are invalid "
        "when another surface fits.\n"
        "- Keep output names faithful; do not relabel surfaces; Vague words "
        "are invalid when several reachable sources fit.\n"
        "- `query.select` includes only returned label fields; every selected "
        "field becomes exact answer. Use `where`/`order_by` for controls. "
        "Do not select helper values unless user asks.\n"
        "- When one answer item combines facts from the same event/record, "
        "follow that path. Avoid independent sibling joins; one grounded "
        ...
        "If a returned key orders/tie-breaks, request must state that order "
        "role; display-only wording is not enough. If roles/surfaces differ "
        "or order is multi-key, user_request must distinguish them; do not "
        ...
        "sequence/id/order wording.",

        "# Feedback And Difficulty-Up Policy\n"
        "FeedbackError is not a new durable instruction source; treat it as a "
        "pointer to an existing named policy plus evidence. Preserve "
        "anchored need/language; change the smallest failing part. Do not "
        "reset to easier/global task or override policy; "
        "one policy source prevents split guidance.\n"
        ...
        "schedule, eligibility check, plan-like list.\n"
        "- Scalar tasks: count, min, max, sum, avg; filter set; "
        "avoid trivial 0/1 results.\n"
        "- List tasks: homogeneous ordered list, fixed limit of 3-5 rows. "
        "Prefer natural "
