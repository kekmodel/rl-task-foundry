...
def build_synthesis_agent_instructions(
    ...
    return "\n\n".join([
        ...
        "numeric. **Aggregate-fn variety** — Type B historically "
        "defaults to `count` (prior 4 accepts all count). Mix it up: "
        "pick `min`/`max`/`sum` when the natural customer phrasing "
        "favors them. Examples (mixed by fn):\n"
        ...
        "**Type B aggregate target is locked across submits.** "
        "The table you count/sum/min/max over on submit 1 cannot "
        "change on later submits. `count(rental)` → `count(payment)`"
        " is NOT an escalation, it is a new task and will be flagged"
        " `difficulty_crank_invalid`. Valid Type B escalations keep"
        " the same aggregate column and add (a) a filter on the "
        "joined child set (`count(rental) where rental_date >= X`),"
        " (b) a composite pair of filters on that set, or (c) "
        "switch `count → sum/min/max` on a numeric/date column of "
        "the same joined table. The target table itself is "
        "immutable.\n"
        "\n"
