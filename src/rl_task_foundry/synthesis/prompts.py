...
def build_synthesis_agent_instructions(
    ...
    return "\n\n".join([
        ...
        "row set, order, and tie-breaks without technical or awkward controls. "
        ...
        "6. Final `query(spec)` answers hidden entity/context before "
        ...
        "source in request/contract or choose another label. If no primary "
        "key, use a primary-key-backed path/aggregate; no hidden path "
        "guessing. Shared everyday nouns need lifecycle/source role in "
        "user_request; do not depend on output_schema field names.",
        ...
        "- Keep request compact. Field keys stay in JSON; "
        "avoid aliases/choices in parentheses. If controls need long "
        "tie-breaks/mechanical field lists, choose another label.\n"
        "- Bind modifiers/filters to exact object/scope/source. Non-null "
        "filters need row-set wording, not output names. If "
        "parent/child rows can match, name the surface; avoid child-row-filter "
        "wording unless query uses it.",
        ...
        "stage/route/sequence/rank) need the same source role used by the "
        "query path; request wording must name it; no normalized choices. "
        "Distinguish source sequence from display rank. Broad words are invalid "
        "when another surface fits.\n"
        "- Keep output names faithful; do not relabel surfaces unless request "
        "names that role. Vague words are invalid when several reachable "
        "sources fit.\n"
        ...
        "If a returned key also orders/tie-breaks, request must state that "
        "order role; display-only wording is not enough. If roles/surfaces "
        "differ or order is multi-key, user_request must distinguish them; "
        "do not bind one vague phrase to multiple concepts.",
        ...
        "field/aggregate; choose another task only if needed. Do not shrink "
        "limits or add hidden handles.\n"
        ...
        "and ask for it in user_request/answer_contract.\n"
        ...
        "schedule, eligibility check, or plan-like list.\n"
        ...
        "deterministic ordering, fixed limit of 3-5 rows. Prefer natural "
