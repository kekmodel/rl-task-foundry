...
def build_synthesis_agent_instructions(
    ...
    return "\n\n".join([
        ...
        "2. Observe values with `sample`, `profile`, or `query`; inspect paths "
        "with `neighborhood`. Do not invent ids or pick small/easy ids. Why: "
        "hidden entity values must be grounded.\n"
        "3. Build the label first: interesting, unique, scoped, visible, "
        "and deterministic.\n"
        "4. Derive `user_request` and `topic` that naturally ask for exactly "
        "that label.\n"
        "5. Final `query(spec)` must answer hidden entity/context, then "
        "submit; inspect diagnostics. Why: the label must be copied from the "
        ...
        "user-facing source surface and canonical query surface must match. "
        "If one concept maps to multiple surfaces, wording binds selected "
        "surface. Bad: request names one surface while label uses another. "
        "Good: request names selected surface. If no primary key, do not expose "
        "row values as labels; use a primary-key-backed path or aggregate. "
        "Why: no hidden path guessing.",
        ...
        "Why: one source of policy prevents split guidance.",

        "# Customer Request\n"
        "Write only the customer's request body in the configured target "
        "language: ask, not answer. Why: user asking.\n"
        "- Use first-person only when hidden context is the requester, account, "
        "session, reservation, order, or records; otherwise use neutral wording.\n"
        ...
        "another entity/path/constraint.\n"
        "- Never invent visible context values; copy dates, names, statuses, "
        "or places only from latest scoped query evidence.\n"
        ...
        "another.\n"
        "- Bind modifiers to the exact object/scope. When parent context and "
        "child rows can both match a phrase, name the surface. Avoid wording "
        "that can also read as a child-row filter unless the query uses it.",
        ...
        "- Verifiable means final `query(spec)` exactly reproduces submitted "
        "JSON; every value is copied from that query.\n"
        "- Unique means one correct structured answer for hidden entity/request. "
        "If visible rows tie, return ties, add a natural visible field, "
        "aggregate, or choose another task; never rely on hidden ids/order/"
        "filters.\n"
        ...
        "- Scope label to hidden current subject/context, directly or through "
        "values derived from it; unrelated global answer is invalid.\n"
        ...
        "when current query evidence marks it user-visible and the request asks "
        "for that reference. Do not make a raw handle the main answer.\n"
        "- Bind answer representation exactly: no code/reference-to-display "
        "or source-field-to-related-field upgrade unless user_request names "
        "that role/representation. Why: multiple answer surfaces are valid.\n"
        "- Keep output names faithful to source meaning. Do not "
        "relabel note/comment/description text as result/status/value unless "
        "the request names that surface. Why: multiple answer columns are "
        "plausible.\n"
        "- In `query.select`, include only fields for the submitted label; "
        "every selected field becomes exact answer. Use `where`/`order_by` for "
        "context, filters, sorting. Do not select helper values unless the "
        "user asks to receive them.\n"
        "- For list labels, returned rows should be distinguishable through "
        "requested output fields. If `query` reports duplicate projected answer "
        "rows, add a natural user-visible distinguishing field, aggregate "
        "duplicates, or choose another task; never add hidden handles.\n"
        ...
        "unrelated child rows. Why: one grounded state, not accidental pairing.\n"
        ...
        "an exact substring of `user_request`. Why: no duplicated evidence.\n"
        ...
        "wording; choose another surface/return ties; never use hidden handles.",

        "# Task Shapes\n"
        "Prefer data-service tasks: history lookup, shortlist, status summary, "
        "schedule, eligibility check, or plan-like list. Why: "
        "arbitrary good DBs.\n"
        ...
        "field; handle-like fields only support natural references.\n"
