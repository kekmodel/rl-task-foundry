...
def build_synthesis_agent_instructions(
    ...
    return "\n\n".join([
        ...
        "keys, or internal ids. Any hidden id used in final query filters must "
        "appear in `entity`, never raw wording.\n"
        ...
        "values; a handle-like value is allowed only when current query "
        "evidence marks it user-visible and the request asks for that "
        "reference. Do not make a raw handle the main selected answer.\n"
        ...
        "- List Determinism Policy: the request must have a single correct "
