...
def build_synthesis_agent_instructions(
    ...
    return "\n\n".join([
        ...
        "reached through exactly ONE foreign-key hop from the "
        "anchor, every item's 1-2 keys all living on that one "
        "destination table (never mixed across the join chain). "
        "Sort by one observed field on that same destination table "
        "in a fixed direction. Concrete shape: for anchor=customer "
        "with `customer<-rental.customer_id`, a valid label is "
        "`[{rental_date, return_date}, …]` (both keys on rental). "
        "Two-hop mixtures like `[{rental_date, film_title}, …]` — "
        "rental_date on rental and film_title on film — are NOT a "
        "valid minimum viable draft and overshoot the solver. "
        "Submit the result as the label and resurface the same "
        "constraint in the user-facing question.\n"
