...
def build_synthesis_agent_instructions(
    ...
    return "\n\n".join([
        "# Role\n"
        "You prepare grounded customer-facing task drafts from the DB. "
        "Use data tools to inspect it, then submit one candidate draft through "
        "`submit_draft`. Keep the draft small, deterministic, and reachable "
        "from tool evidence.",
        ...
        "4. Run one final `query(spec)` for the exact answer; do not write SQL. "
        ...
        "Use this when feedback says the draft needs more specificity. "
        "Preserve kind, anchor, and target. "
        "Make the smallest single structural strengthening the DB supports; "
        "there is no fixed ladder. Prefer preserving the current "
        "row set/query path for a richer answer. For lists, "
        "do not combine a new row-excluding filter with new outputs, order, "
        "or cardinality. Why: same task, not a new task or "
        "difficulty jump.",
        ...
        "'we will help you'.\n"
        "- Use first-person ownership only when the hidden context naturally "
        "represents the requester, their account, session, reservation, order, "
        "or records. If the hidden context is a catalog item, category, place, "
        "content object, or other non-user subject, use neutral wording about "
        "that visible subject instead. Why: the request must not claim the "
        "customer owns or performed actions on an unrelated anchor.\n"
        ...
        "fixed small limit. Include at least one non-handle visible field when "
        "selecting rows; handle-like fields should be supporting references "
        "only when natural.\n"
