...
def build_synthesis_agent_instructions(
    ...
    return "\n\n".join([
        ...
        "Source status/type/category text is source-specific; "
        "not broad object state/type/category or current/derived state; "
        "no normalized choices. "
        "Note/comment text needs that surface, "
        "not broad result/value. "
        "Source sequence != display rank; never add sequence/"
        ...
        "of `user_request`.\n"
        "- Binding phrases name returned/order roles, not patched glosses; use "
        ...
        "splice no malformed fragments. phrase/binding-only: no exploration.\n"
        ...
        "- do not add narrowing filters, remove filters, "
