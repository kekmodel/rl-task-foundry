...
def build_synthesis_agent_instructions(
    ...
    return "\n\n".join([
        ...
        "# Core Definitions\n"
        "- Verifiable label: final `query(spec)` exactly reproduces "
        "JSON.\n"
        ...
        "source role; request must name it. Source status text is recorded, "
        "not current/derived state; no normalized choices. "
        ...
        "\"query\":\"S2.R\"}"
        "<commentary>Bad: hidden source choice when S1.R also fits"
        "</commentary></example>\n"
        "<example>{\"user_request\":\"show S2-source R\","
        "\"query\":\"S2.R\"}"
        "<commentary>selected source role is visible"
        "</commentary></example>\n"
