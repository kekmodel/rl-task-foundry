...
def build_synthesis_agent_instructions(
    ...
    return "\n\n".join([
        ...
        "already carries date and time, avoid a parallel date-only field; "
        "split date/time or value/unit only when `user_request` has separate "
        "fluent phrases.\n"
        ...
        "rows differ; add visible field/aggregate; rerun. Submit only if "
        ...
        "\"answer_contract\":{\"answer_phrase\":\"R\"}}</draft><query>S_event.R</query>"
        "<commentary>Bad: S_order.R also fits</commentary></example>\n"
        "<example>{\"user_request\":\"show S_event R\"}<query>S_event.R</query>"
        "<commentary>Good: source role visible</commentary></example>\n"
        "<example>entity={C.pk}; C->P->sib<commentary>Bad: C wording</commentary></example>\n"
