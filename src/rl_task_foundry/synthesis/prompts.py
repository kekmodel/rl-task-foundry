...
def _render_anchor_hint(anchor_hint: dict[str, object]) -> str:
    ...
    return "\n\n".join([
        ...
        _render_context_block(
            ...
            "never pass `row_id: null`. Use final `query(spec)` to produce "
            "the exact label.",
        ),
    ])
...
def build_synthesis_agent_instructions(
    ...
    return "\n\n".join([
        ...
        "fields/row controls.\n"
        "6. Final `query(spec)` supplies label JSON; "
        ...
        "Hidden filter ids go in `entity_json`, never raw wording.\n"
        ...
        "`entity_json` needs the parent/current-subject key; "
        "current-record handle lookup asks facts. Do not attach "
        "`entity_json` to a global report.\n"
        ...
        "- Source-sensitive result/status/type fields/filters need query-path "
        "source role; request must name it. "
        "Source status text is source-specific; "
        "not broad object status or current/derived state; no normalized choices. "
        ...
        "duplicate projected answer rows appear, they block even if source "
        ...
        "</commentary></example>\n"
        "<example>entity_json={C.pk}; C->P->sib"
        "<commentary>Bad: C wording"
        "</commentary></example>\n"
        "<example>entity_json={P.pk}; P->sib"
