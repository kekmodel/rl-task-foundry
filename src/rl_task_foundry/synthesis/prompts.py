...
def build_synthesis_agent_instructions(
    ...
    return "\n\n".join([
        ...
        "# Workflow\n"
        "1. Map: call `schema_map`; choose a plausible root. Why: the DB "
        "decides the domain.\n"
        "2. Observe: use `sample`, `profile`, or `query`; use `neighborhood` "
        "for reachability and readable surfaces. Do not invent ids or pick "
        "small/easy ids. Why: hidden entity values must be grounded.\n"
        "3. Build the label first: interesting, unique, verifiable, scoped, "
        "visible, deterministic.\n"
        "4. Derive `user_request` and `topic` from the label; the request must "
        "naturally ask for exactly the label fields and row controls.\n"
        "5. Final `query(spec)` answers hidden entity/context, runs immediately "
        "before `submit_draft`, and supplies copied label JSON. Inspect "
        "diagnostics. Why: no global answer with a decorative entity.",
        ...
        "bind the selected role or choose another. If no primary key, do not "
        ...
        "global report.\n"
        "- If an entity/filter has only a hidden handle and no observed "
        "customer-visible surface, do not expose it; choose another path or "
        "constraint.\n"
        "- Copy visible context values (dates, names, statuses, places) only "
        "from latest scoped query evidence.\n"
        "- Bind modifiers to exact object/scope. If parent and child rows can "
        "both match a phrase, name the surface; avoid child-row-filter wording "
        "unless the query uses it.",

        "# Label Contract\n"
        "Label is the structured result that answers `user_request`, not final "
        "prose. Why: exact structured labels make the task verifiable.\n"
        "- `answer_contract.kind='scalar'` only for aggregate answers "
        "(`count/sum/avg/min/max`); `list` for selected rows or lookup fields. "
        "Why: scalar is one aggregate.\n"
        ...
        "- Use stable API-style field names and user-visible non-handle values. "
        "Handle-like values only support natural references when query marks "
        "them user-visible and request asks for that reference; never make a "
        "raw handle the main answer.\n"
        ...
        "role/representation. Why: multiple answer surfaces are valid.\n"
        ...
        "names that surface. Why: multiple answer columns are plausible.\n"
        ...
        "sibling joins that can pair unrelated child rows. Why: one grounded "
        "state, not accidental pairing.\n"
        "- `answer_contract` only binds request phrases to label/order: kind, "
        "answer_phrase, constraint_phrases, limit_phrase, output/order "
        ...
        "- Binding phrases must name the returned field's role. If two fields "
        "come from different roles/surfaces with similar names, user_request "
        "must distinguish those roles before both fields are labeled; do not "
        ...
        "equivalent. Why: exact verification.\n"
        ...
        "there is no fixed ladder. Why: same task, not a new task or difficulty "
        "jump.\n"
        "- For lists, keep filters, order, limit, row set, and output "
        "fields/source meanings; append exactly one user-visible field from "
        "the same row path/direct relationship, and ask for it in "
        ...
        "schedule, eligibility check, or plan-like list. Why: arbitrary good "
        "DBs.\n"
        "- Scalar tasks: one aggregate such as count, min, max, sum, or avg; "
        "filter large sets and avoid trivial 0/1 results.\n"
        "- List tasks: homogeneous ordered list with visible fields, explicit "
        "filters, deterministic ordering, fixed small limit. For event/log "
        ...
        "field or relationship at a time. Why: calibration.\n"
