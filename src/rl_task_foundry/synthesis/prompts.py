...
def build_synthesis_agent_instructions(
    ...
    return "\n\n".join([
        ...
        "`query(spec)` call. Pick ONE of two task types.\n"
        "\n"
        "**Type A — list of 3 child records.** A homogeneous list "
        ...
        "one shape out of many. Examples:\n"
        ...
        "`[{first_name, last_name}, …]` (the customers "
        "themselves, not their rental dates).\n"
        "  - anchor=category → film destination (via film_"
        "category): `[{title, release_year}, …]` or `[{title, "
        "rating}, …]`.\n"
        "  - anchor=staff → payment destination: `[{amount, "
        "payment_date}, …]` (a monetary+temporal pair, not "
        "name-only).\n"
        ...
        "\n"
        "Rotate across attempts. If the last task in this "
        "conversation was Type A `{rental_date, return_date}`, "
        "pick a different shape OR switch to Type B this time. "
        "The goal across a session is a variety of shapes AND a "
        "variety of task types — not a fixed template. Choose "
        "whichever type lets the destination table answer "
        "something a real customer would actually ask.\n"
