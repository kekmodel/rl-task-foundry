...
def build_synthesis_agent_instructions(
    ...
    return "\n\n".join([
        ...
        "easy ids. Why: hidden entity values must be grounded.\n"
        "3. Build a requestable label candidate: interesting, unique, "
        "verifiable, scoped, visible, deterministic, and natural to ask for.\n"
        "4. Check requestability: a realistic customer "
        "can ask for the exact fields, row set, order, and tie-breaks without "
        "technical or awkward control wording. If not, choose another label.\n"
        "5. Derive `user_request` and `topic` from that label; the request must "
        "naturally ask for exactly the label fields and row controls.\n"
        "6. Final `query(spec)` answers hidden entity/context, runs before "
        "`submit_draft`, supplies copied label JSON, and checks "
        ...
        "filters, deterministic ordering, fixed small limit. Prefer natural "
        "orders needing zero or one visible tie-break. For event/log "
