...
def build_synthesis_agent_instructions(
    ...
    return "\n\n".join([
        ...
        "Reach a readable destination table by the shortest FK "
        "chain from the anchor (single hop for leaf anchors like "
        "customer/rental/film; the natural chain for hub anchors "
        "like city/category). Every record's 1-2 keys must all "
        "live on that ONE destination table — never mix columns "
        "from two tables inside one record. Sort by one observed "
        "field on that same destination table in a fixed direction.\n"
        "\n"
        "Pick keys that match the destination table's natural "
        "readable surface. Do NOT default to `{rental_date, "
        "return_date}` regardless of anchor; that is just one "
        "shape out of many. Varied destination-appropriate "
        "examples:\n"
        "  - anchor=customer → rental destination: "
        "`[{rental_date, return_date}, …]`.\n"
        "  - anchor=film → actor destination (via film_actor): "
        "`[{first_name, last_name}, …]`.\n"
        "  - anchor=city → customer destination (via address): "
        "`[{first_name, last_name}, …]` (the customers themselves, "
        "not their rental dates).\n"
        "  - anchor=category → film destination (via "
        "film_category): `[{title, release_year}, …]` or "
        "`[{title, rating}, …]`.\n"
        "  - anchor=staff → payment destination: "
        "`[{amount, payment_date}, …]` (a monetary+temporal pair, "
        "not name-only).\n"
        "Choose keys that answer something a real customer would "
        "ask about that destination entity — names for people, "
        "titles for films, amounts for payments, dates only when "
        "dates are the point. Rotate across attempts: if the last "
        "task in this conversation used `{rental_date, "
        "return_date}`, do NOT pick it again unless the anchor "
        "genuinely makes that the natural answer.\n"
        "\n"
        "Cross-table record shapes like `[{rental_date, "
        "film_title}, …]` — rental_date on rental and film_title "
        "on film in the same record — are NOT a valid minimum "
        "viable draft; the solver would have to join two tables "
        "per answer row and overshoot. Submit the result as the "
        "label and resurface the same constraint in the user-"
        "facing question.\n"
