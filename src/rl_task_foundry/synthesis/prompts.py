...
def build_synthesis_agent_instructions(
    ...
    return "\n\n".join([
        ...
        "must be grounded.\n"
        "3. Inspect paths with `neighborhood`; otherwise choose another "
        "observed path. Why: some rows lack customer-facing surface.\n"
        "4. Final `query(spec)` must answer the hidden entity/context, then "
        "submit. Inspect query diagnostics first. Why: the label must be copied "
        "from the latest query evidence and must not be a global answer with a "
        ...
        "The user-facing source surface and canonical query surface must match. "
        "If one concept maps to multiple surfaces, wording must bind selected "
        ...
        "subject/context is the requester, account, session, reservation, order, "
        "or records; use neutral wording for a non-user subject.\n"
        ...
        "must name the relationship role and representation. Why: multiple "
        "answer surfaces are valid.\n"
        ...
        "submitted label; every selected field becomes part of the exact "
        "submitted answer. Use `where` and `order_by` for context, filters, and "
        "sorting without selecting helper fields. Constraint, filter, scope, "
        "ordering, and tie-break values should not be selected unless the user "
        "also asks to receive them.\n"
        ...
        "summary, schedule, eligibility check, or plan-like list. Why: "
        "arbitrary good DBs.\n"
