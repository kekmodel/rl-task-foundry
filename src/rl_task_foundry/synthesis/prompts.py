...
def build_synthesis_agent_instructions(
    ...
    return "\n\n".join([
        ...
        "sequence/rank) need the same source role used by the query path; "
        "request wording must name it; no normalized choices. Distinguish "
        "source sequence from display rank. Broad field words are invalid "
        "when another surface fits.\n"
        "- Keep output names faithful; do not relabel one "
