...
def build_synthesis_agent_instructions(
    ...
    return "\n\n".join([
        ...
        "# Workflow\n"
        "1. Start with `schema_map`, then choose a plausible root table. Why: "
        "the DB decides the domain; no task shape is hard-coded.\n"
        "2. Use `sample`, `profile`, or `query` to observe real entity values. "
        "Do not invent ids or choose a row only because its id is small/easy. "
        "Why: hidden entity values must be grounded.\n"
        "3. If the path looks usable, inspect it with `neighborhood`; otherwise "
        "choose another observed path. Why: some rows lack customer-facing task "
        "surface.\n"
        "4. Run final `query(spec)` for the exact answer; do not write SQL. "
        "Scope it to the hidden entity or context derived from it, then submit. "
        "Why: the label must be copied from the latest query evidence and must "
        "not be a global answer with a decorative entity attached.\n"
        ...
        "row set/query path. For lists, "
        "first try keeping all existing filters, order, limit, row set, and "
        "output fields, then add exactly one user-visible output field from "
        "the same row path or a direct relationship. Do not shrink the fixed "
        ...
        "language. The request is the ask, not the answer. Why: it should look "
        "like a real user asking an agent for information.\n"
        "- Use the customer's own voice: first-person (`my records`) or direct "
        "request wording. Avoid service-side phrasing such as "
        "'we will help you'.\n"
        "- Use first-person ownership only when the hidden context represents "
        "the requester, account, session, reservation, order, or records. If it "
        "is a catalog item, place, content object, or other non-user subject, "
        "use neutral wording. Why: the request must not claim unrelated "
        "customer ownership or action.\n"
        "- The customer does not know DB tables, rows, primary keys, foreign "
        "keys, or internal ids. Any hidden id used in final query filters "
        "must appear in `entity`, never raw wording.\n"
        "- Every draft must be about the hidden current subject/context or "
        "derived values. Do not attach `entity` to a global report; "
        ...
        "relationship roles or sibling tables, bind wording to the selected "
        "source role or choose another. Why: ambiguous paths create "
        "multiple answers and weak RL signal.",
        ...
        "selected fields are row/list results.\n"
        ...
        "request naturally asks for that reference, not for a raw DB id. Do "
        ...
        "sorting without selecting those helper fields. Why: every selected "
        "field becomes part of the exact submitted answer. Constraint, filter, "
        "scope, ordering, and tie-break values should not be selected unless "
        "the user also asks to receive those values.\n"
        ...
        "unrelated child rows. Why: exact labels must describe one unique "
        "grounded state, not an accidental combination.\n"
        ...
        "small plan-like list. Why: code adapts to arbitrary "
        "good DBs.\n"
        ...
        "filters, deterministic ordering, and a fixed small limit. For "
        "event/log tables, use first/latest/top 3-5 rows and put the limit in "
        "request; avoid all matching when observed count exceeds 5. "
        "Include at least one non-handle visible field; handle-like fields are "
        "supporting references only when natural.\n"
        "- Keep initial row/list labels narrow: prefer 3-4 fields, max 5 "
        "before feedback. Do not turn one-row lookup into a wide profile. If "
        "too easy, use the Difficulty-Up Policy to add one coherent field or "
        "relationship at a time. Why: land near calibration.\n"
        "- Open-ended recommendations are allowed only after deterministic "
        "filters, thresholds, ordering, limit, and tie-breaks. Submit the "
        "candidate through `submit_draft` and follow "
