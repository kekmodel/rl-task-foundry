...
def build_synthesis_agent_instructions(
    ...
    return "\n\n".join([
        ...
        "verifiable, scoped.\n"
        "4. Check requestability: realistic customer asks without technical "
        "or awkward controls/formatting quirks; "
        ...
        "# Core Definitions\n"
        "- Verifiable label: final `query(spec)` exactly reproduces JSON.\n"
        "- Unique label: one correct structured answer; "
        "never rely on hidden ids/order/filters.\n"
        "- Source surface: user wording, label fields, query path; "
        "reachable surfaces: if same broad noun returns "
        ...
        "- No DB/table/column jargon or technical sequences; use domain roles. "
        ...
        "lookup. Do not attach `entity` to a global report.\n"
        "- Hidden handle: do not expose it. "
        "Copy scoped evidence exactly; no translate/transliterate.\n"
        "- Keep request compact. Exact substring "
        "binding never justifies broken wording, invented/diagnostic phrases, "
        "or key translations. Field keys stay JSON. Fields use include/with; "
        "order/sort only for row order. If roles/tie-breaks "
        "cannot fit one natural request, choose another label.\n"
        "- Bind modifiers/filters to exact object/scope/source. Non-null/status/"
        "type/category filters need row-set wording and matching query.where, "
        "not output names. If only returned, ask records plus field.",
        ...
        "user-visible non-handles by metadata, not names; "
        "handle refs need query/user visibility; "
        ...
        "- Source-sensitive result/status/type/category/classification fields/"
        "filters need query-path source role; request must name it. "
        "Source status/type/category text is source-specific; not broad object "
        "state/type/category or current/derived state; no normalized choices. "
        "Note/comment text needs that surface, not broad result/value, and "
        "never add sequence/order output only to make duplicate rows unique. "
        "Keep output names faithful; broad/vague "
        "words are invalid when several reachable sources fit.\n"
        "- `query.select` includes only returned label fields; every selected field "
        "becomes exact answer. Do not select helper values unless asked. "
        "One natural output slot maps to one label field; if one timestamp "
        "already carries date and time, split date/time or value/unit only when "
        "`user_request` has separate "
        "fluent phrases.\n"
        "- When one answer item combines facts from the same event/record. "
        "Avoid independent sibling joins.\n"
        ...
        "of `user_request`.\n"
        "- Binding phrases name returned/order roles; use "
        ...
        "words and direction agree (newest/latest vs oldest/earliest). "
        "One list order; no selection/display split.\n"
        "- Rows distinguishable through requested output fields. If "
        "duplicate projected answer rows, block even if source "
        "rows differ; add visible field/aggregate. Submit only if "
        "diagnostics clear; if still duplicate or only sequence/reference/order "
        "can distinguish them, switch label. Do not shrink limits or add hidden "
        "handles.\n"
        ...
        "tied rows; if two keys still tie, switch label. Display field "
        "wording is not row-order wording; every `query.order_by` key needs "
        "its own order phrase in `user_request`/`answer_contract.order_bindings`. "
        "Source sequence/order numbers are technical unless that is the "
        "domain answer; never add them for tie-break/binding repair. Max two "
        "order keys.",

        "# Scope Examples\n"
        "{\"user_request\":\"show R\","
        "\"answer_contract\":{\"answer_phrase\":\"R\"}}<query>S_event.R</query>"
        "<commentary>Bad: S_order.R also fits</commentary>\n"
        "{\"user_request\":\"show S_event R\"}<query>S_event.R</query>"
        "<commentary>Good: source role visible</commentary>\n"
        "entity={C.pk}; C->P->sib<commentary>Bad: C wording</commentary>\n"
        "entity={P.pk}; P->sib<commentary>Good: rows share parent</commentary>",
        ...
        "no malformed fragments. phrase/binding-only: no exploration.\n"
        ...
        "# Task Shapes\n"
        "- Scalar: count/min/max/sum/avg; avoid trivial 0/1.\n"
