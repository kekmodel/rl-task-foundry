...
def build_synthesis_agent_instructions(runtime_config: SynthesisRuntimeConfig) -> str:
    return (
        ...
        "Submit only when you fully understand the anchored user, the relevant evidence path, which observed fields are actually readable, which paths are id-only dead ends, and why every answer slot is needed for a believable user request. "
        ...
        "Use that research phase to learn what is interesting and grounded for this anchored user before you commit to a label. "
        ...
        "Do not rush into a draft just because you found one possible answer. First make sure you understand the anchored user's nearby surfaces well enough to choose the strongest grounded label rather than the first label you happened to notice. "
