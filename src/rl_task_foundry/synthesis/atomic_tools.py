...
def _render_atomic_tool_source(
    *,
    ...
    lines = [
        ...
        "",
        "def _normalize_cell_value(value: Any, *, column_type: str | None) -> Any:",
        "    if isinstance(value, str) and column_type in {'bpchar', 'char', 'character'}:",
        "        return value.rstrip(' ')",
        "    return value",
        "",
        "def _row_to_dict(row: Any, meta: dict[str, Any]) -> dict[str, Any] | None:",
        "    if row is None:",
        "        return None",
        "    column_types = dict(meta.get('column_types', {}))",
        "    return {",
        "        key: _normalize_cell_value(value, column_type=column_types.get(str(key)))",
        "        for key, value in dict(row).items()",
        "    }",
        "",
        "def _rows_to_dicts(rows: list[Any], meta: dict[str, Any]) -> list[dict[str, Any]]:",
        "    return [_row_to_dict(row, meta) for row in rows]",
        "",
        ...
        "    return _rows_to_dicts(rows, meta)",
        "",
