...
from rl_task_foundry.tasks import factory as factory_module
...
async def test_task_factory_does_not_fallback_aggregate_sampling_to_count_one(monkeypatch):
    ...
    monkeypatch.setattr(factory_module.asyncpg, "connect", _fake_connect)

    anchor_values = await factory._sample_anchor_values(graph, path, contract, limit=3)
