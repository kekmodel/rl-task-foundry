...
@pytest.mark.asyncio
async def test_submit_draft_rejects_unexpressed_visible_filter_value(
    tmp_path: Path,
) -> None:
    controller = SubmitDraftController(
        config=_config_with_synthesis_output(tmp_path),
        requested_topic="input_events",
        solver_orchestrator=_FakeSolverOrchestrator(
            matched_solver_runs=30,
            total_solver_runs=30,
        ),
    ...
    _seed_min_initial_exploration(controller)
    anchor_entity = {"stay_id": 31338022}
    label = [
        {
            "administration_start_time": "2176-11-29T15:00:00",
            "item_label": "PO Intake",
            "amount": 120.0,
            "amount_unit": "ml",
        }
    ]
    payload = SubmitDraftPayload.model_validate(
        {
            ...
            "question": _wrap_user_prompt(
                anchor_entity,
                "중환자실 입원 기간 동안 투여된 약물 기록 중 가장 최근 1개를 알려주세요.",
            ),
            "answer_contract": _list_answer_contract(
                phrase="투여된 약물 기록",
                constraint_phrases=["중환자실 입원 기간 동안"],
                limit_phrase="가장 최근 1개",
            ),
        }
    )
    _record_query_evidence(
        ...
        referenced_columns=[
            ...
            },
            {
                ...
                "is_handle": False,
                "op": "eq",
                "value": "FinishedRunning",
            },
        ],
        query_params={"spec": {"limit": 1, "where": [{"value": 31338022}]}},
    )

    message = await controller.submit(payload)

    assert "user-visible value that is not present in user_request or label" in message
    assert "hidden membership filter" in message
    assert controller.last_feedback_error_codes == (
        "answer_contract_filter_unexpressed",
    )
    assert controller.attempts == []
    assert controller.accepted_draft is None
