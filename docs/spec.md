...
- probe runtime은 stage별 `answer` / `facts` read count도 기록해 dead-code 스타일 weak verifier를 diagnostics로 남긴다
...
- solution/verifier tool call 수와 stage별 runtime read count도 structured diagnostics로 수집된다
...
- v1 runtime은 `max_consecutive_category_discards`와 `category_backoff_duration_s`로 이를 로컬 runtime state에서 enforce한다
- runtime은 scheduler handoff용으로 `category_status()` snapshot을 노출하고, 마지막 discard outcome / error codes / backoff remaining을 함께 제공한다
- v1 `SynthesisOrchestrator`는 registry entry 목록을 받아 cached single-db runtime들에서 snapshot을 모으고, scheduler decision이 `READY`일 때만 해당 db runtime에 draft 생성을 위임한다
- 아직 선택되지 않은 DB는 runtime이 instantiated되기 전까지 empty status snapshot으로 취급한다
- v1 `SynthesisRegistryRunner`는 registry entry 집합을 checkpoint namespace와 함께 순회하며, 성공한 `db_id x category` pair를 durable checkpoint에 기록해 resume 가능하게 만든다
- runner summary는 `COMPLETED_ALL / MAX_STEPS_REACHED / ALL_BACKED_OFF / EMPTY_REGISTRY` outcome을 노출하고, full draft 객체 대신 lightweight step summary만 유지한다
...
- v1 runtime은 artifact retry 간 `difficulty_vector` 축이 줄어들거나 사라지면 weakening으로 reject한다
