...
- synthesis agent는 tool이 아니라 task contract, solution, verifier, shadow verifier, instance space를 합성하고 runtime이 per-instance `rendered_user_prompt`를 materialize한다
- reward path는 canonical answer + exact-match reward로 수렴하고, synthesis-time verifier contract는 staged form을 유지한다
...
- synthesis agent가 atomic tool bundle을 참조하는 proposed environment와 audit artifact source를 생성한다
...
- drop priority는 `aggregate -> like -> distinct -> range` 순이다
...
- 2026-04-12 기준 현재 `sakila` + `public` schema + 현재 visibility/compression 정책에서는 atomic tool 108개가 생성된다
- family 분포는 `T1=28`, `T2=36`, `T4=44`였다
- 즉 실제 tool count는 schema와 visibility profile의 함수이며, `max_tool_count=256`은 상한이지 목표값이 아니다
...
3. synthesis agent에는 `"available atomic tools: [...]"` context만 제공한다
4. agent가 `question`, `output_schema`, `solution.py`, `verifier.py`, `shadow_verifier.py`, `instance_space`를 생성한다
5. registration policy가 `solution.py`, `verifier.py`, `shadow_verifier.py`가 atomic tool set 내 이름만 호출하는지 AST 검증한다
6. self-consistency와 triple oracle 검증을 수행한다
7. runtime이 per-instance `rendered_user_prompt`와 canonical answer를 materialize한다
8. environment registry에 commit한다
...
v1 self-consistency는 schema/category phase를 재사용하면서 artifact generation만 재시도한다.

- schema exploration과 category inference는 한 번만 수행한다
- artifact generation만 `attempt_index=1..N`으로 재실행한다
- registration 실패 diagnostics는 다음 attempt input으로 들어간다
- self-consistency 실패 diagnostics도 다음 attempt input으로 들어간다
- registration과 triple oracle을 모두 통과한 attempt만 materialize된다

운영 규칙:

- `max_self_consistency_iterations`를 가진다
- tool set이 바뀌면 solution/verifier/shadow를 모두 invalidate한다
...
synthesis-time verifier contract는 현재 staged fact checker 구조를 유지한다.

핵심 가정:

- 각 instance에는 canonical answer가 정확히 하나 존재한다
- 이 canonical answer는 `solution.py`, `verifier.py`, `shadow_verifier.py`의 triple oracle 합의로 생성된다
...
- `shadow_verifier.py`
  - verifier와 동일한 staged contract

즉 2026-04-12 기준 triple oracle은 “세 module이 모두 canonical answer를 직접 계산한다”가 아니라:

- `solution.py`가 candidate canonical answer를 계산하고
- `verifier.py`, `shadow_verifier.py`가 그 candidate answer를 독립적으로 검증하는 구조다

self-consistency는 아래를 모두 요구한다.

- solution output이 schema-driven canonicalization을 통과한다
- primary verifier가 그 answer를 accept한다
- shadow verifier가 그 answer를 accept한다

참고:

- `compute_canonical_answer(tools) -> dict` 단일-entrypoint verifier는 spec target으로 논의되었지만, 아직 authoritative implementation이 아니다
- 현재 authoritative verifier contract는 staged `fetch_facts -> facts_match_answer_claims -> check_constraints -> verify`다
...
- 2026-04-12 기준 registry commit path에 hard-wired 된 acceptance gate는 `code registration -> self-consistency -> registry dedup/materialization`까지다
- cross-instance consistency와 solver pass-rate band는 bundle/orchestrator surface와 calibration primitive는 구현됐지만, synthesis registry runner의 mandatory acceptance gate로는 아직 연결되지 않았다
...
- `canonical_answers.jsonl`
- `databases/{db_id}/atomic_tool_definitions.json`
- `audit/solution.py`, `audit/verifier.py`, `audit/shadow_verifier.py`
