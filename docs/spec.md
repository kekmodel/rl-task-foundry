...
- synthesis agent는 tool이 아니라 `rendered_user_prompt`, solution, verifier, shadow verifier, instance space를 합성한다
- verifier path는 canonical answer 계산과 exact-match reward로 수렴한다
...
4. agent가 `rendered_user_prompt`, `output_schema`, `solution.py`, `verifier.py`, `shadow_verifier.py`, `instance_space`를 생성한다
...
7. per-instance canonical answer를 materialize한다
...
runtime는 deterministic sampler로 instance를 만들고, 각 instance에 대해 unique canonical answer를 materialize한다.

## Verification and Reward Model

verification은 staged fact checker가 아니라 canonical answer exact match 기반으로 단일화한다.
...
기존 Hybrid Rule A/B/C/D는 아래 구조로 재정의한다.

- `solution.py`
- `verifier.py`
- `shadow_verifier.py`

세 artifact는 각자 독립적으로 canonical answer를 계산한다.

```python
def solve(tools) -> dict

def compute_canonical_answer(tools) -> dict
```

요구:

- solution, verifier, shadow verifier가 모두 canonical answer를 산출한다
- 세 canonical answer가 exact match로 모두 일치해야 self-consistency를 통과한다
- 불일치 시 아래 diagnostic을 기록한다
  - `solution_vs_verifier_mismatch`
  - `verifier_vs_shadow_mismatch`
  - `solution_vs_shadow_mismatch`

즉 reward 계산은 runtime/server의 pure function이고,
generated Python은 canonical answer materialization 단계까지만 책임진다.
...
기존 `question` 필드는 authoritative actor surface가 아니라 internal fragment 또는 legacy field로만 본다.

`output_schema`는 verifier 내부 계약으로 이동한다.
...
새 verifier contract의 단일 entrypoint는 아래다.
...
`VerifierContract`는 이제 내부용 `output_schema`를 포함한다.

### ShadowVerifierContract

shadow verifier는 같은 signature를 따른다.

- `compute_canonical_answer(tools) -> dict`
...
아래 contract는 legacy deletion 대상이다.
...
6. solver backend는 `task.question`, `task.answer_schema`, `task.language`를 직접 참조하지 않는다
